...
SQLMesh 元数据不存储在 MaxCompute 中。生产环境的元数据应通过独立的 `state_connection` 管理，推荐使用 Postgres。MaxCompute 只作为模型 SQL、audit 查询和物理表/视图操作的执行引擎。
...
- 如果经过 MaxCompute 验证可用，使用 `CREATE OR REPLACE VIEW`。
- 否则使用确定性的 `DROP VIEW IF EXISTS` 后接 `CREATE VIEW`。
...
当模型将时间范围映射到 MaxCompute 分区写入时支持。
...
第一版应聚焦分区 overwrite 行为。delete/merge 型增量策略不在范围内，因为它们依赖的引擎能力和表格式不属于稳定的 MaxCompute 离线主路径。
...
- `schema` 映射为 MaxCompute schema/namespace，如果目标环境启用了 schema 能力。
...
- `execution_mode: Literal["offline", "maxqa"]`
...
- 文档中应展示 MaxCompute execution connection 与 Postgres state connection 的组合配置。
...
- `table_exists(table_name)`
  - 使用 PyODPS table lookup 或 reload 行为。
  - 对 MaxCompute not-found 错误返回 `False`。
  - 在确认 PyODPS 异常类后，避免过宽捕获无关异常。
...
- 一个小型 SQLMesh 示例项目或内部集成 fixture。
- 使用真实 MaxCompute 凭证的手动或 CI-gated smoke test。

验收标准：

- `sqlmesh plan` 成功。
- `sqlmesh apply` 成功。
- 配置的 MaxCompute project/schema 下生成预期表/视图。
...
| schema/project 语义因 MaxCompute 环境差异而变化 | 对象解析错误 | 将 `project` 视为 catalog，将 `schema` 视为可选 namespace，并使用真实账号验证 |
...
- `CREATE OR REPLACE VIEW` 是否在所有目标 MaxCompute project 中都安全，或第一版是否应统一使用 drop/create。
- PyODPS 中 table-not-found 和 schema-not-found 的准确异常类。
- `execution_mode = "maxqa"` 是否需要不同的 PyODPS connection 或 hint 行为。
- struct 等更复杂 MaxCompute 类型的最终 SQLGlot 映射边界。
